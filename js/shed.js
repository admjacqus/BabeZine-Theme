function timeit() {
  ...
  jQuery(".timeit").each(function() {
    ...
    function parseISO8601(d) {
      var isoExp = /^\s*(\d{4})-(\d\d)-(\d\d)?.(\d\d)?.(\d\d)\s*jQuery/,
        date = new Date(NaN),
        datenew,
        month,
        ...
        if (returndate == "Invalid Date" || !timetest) {
          ...
          jQuery(obj)
            ...
            .attr(
              ...
              '"' +
                date +
                '" date is incorrect; please use YYYY-MM-DD HH:MM format'
            );
        }
  ...
  if (next_run_array.length > 0) {
    ...
    // console.log(
    //   "next run of timeit function is in " + nextruntime / 1000 + "seconds"
    // );
    setTimeout(function() {
      timeit();
    }, nextruntime);
