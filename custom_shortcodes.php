...
function product_func( $atts ) {
    ...
    if ($product_sale->length > 0) {
        ...
        foreach ($product_sale as $pat) {
            ...
            //img url containing img number
            $product_src = $product_xpath->query('(//img[contains(@class,"js-gallery--image-thumb")])['. $imgNumber .']/@src', $pat)->item(0)->nodeValue;
            //strip the amp transformation
            $findme = '?';
        ...
        foreach ($product as $pat) {
            ...
            $pos = strpos($product_src, $findme);
            if ($pos === false) {
                echo "The string '$findme' was not found in the string '$product_src' of img # '$imgNumber'";
            } else {
                //then add amp transformation we want
                $thumb_size = '?$product-page__thumbnail--3x$';
                $product_img = substr($product_src, 0, $pos) . $thumb_size;
